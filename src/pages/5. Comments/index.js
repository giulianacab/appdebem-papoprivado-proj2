...
export function Comments() {
        ...
        async function fetchComments() {
            try {
                const response = await axios.get(
                    `https://ironrest.herokuapp.com/tswcomments/${_id}`
                );
    ...
    const saveComments = async (comments) => {
        ...
        try {
            const response = await axios.put(
                `https://ironrest.herokuapp.com/appdebem-comments/${showEditInput}`,
                { comment: comments }
            );
    ...
    async function handleSubmit(e) {
        ...
        try {
            const response = await axios.put(
                `https://ironrest.herokuapp.com/appdebem-comments/${_id}`,
                form
            );
    ...
    async function handleDelete(_id) {
        try {
            await axios.delete(`https://ironrest.herokuapp.com/appdebem-comments/${_id}`);
    ...
    const [editComments, setEditComments] = useState(comments.length > 0 && showEditInput ? comments.find(comment => comment._id == showEditInput).comment : '');
    ...
    function showInput(_id) {
        setShowEditInput(_id)
    }
                    ...
                    {comments.map((currentComment) => {
                        if (showEditInput === false || showEditInput !== currentComment._id) {
                            return <>
                                ...
                                <input value={currentComment.comment}></input>
                            ...
                            return <>
                                <input
                                    name="comment"
                                    value={editComments}
                                    onChange={({ target }) => { setEditComments(target.value) }}
                                />
                                <button className={style.buttonComments} onClick={() => saveComments(editComments)}>Salvar comentário</button>
                            </>
                        }
                    })}
                </div>
